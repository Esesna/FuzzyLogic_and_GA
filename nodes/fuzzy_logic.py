...
class FuzzyLogic():
    ...
    def initOAFLC(self):
        range_front = ctrl.Antecedent(np.arange(0, 2, 0.001), "front")
        range_front['near'] =       fuzz.trapmf(range_front.universe, [                 0,                  0, self.paramForFL[0], self.paramForFL[1]])
        ...
        range_left = ctrl.Antecedent(np.arange(0, 5, 0.001), "left")
        range_left['near'] =       fuzz.trapmf(range_left.universe, [                 0,                  0, self.paramForFL[4], self.paramForFL[5]])
        ...
        range_right = ctrl.Antecedent(np.arange(0, 5, 0.001), "right")
        range_right['near'] =       fuzz.trapmf(range_right.universe, [                 0,                  0, self.paramForFL[4], self.paramForFL[5]])
    ...
    def initTFLC(self):
        error_in_angle = ctrl.Antecedent(np.arange(-180, 180, 0.01), "ErrorInAngle")
        ...
        distance = ctrl.Antecedent(np.arange(0, 2, 0.001), "distance")
        distance['near'] =       fuzz.trapmf(distance.universe, [0, 0, 0, 1])
        distance['far'] =        fuzz.trapmf(distance.universe, [ 0,  1,  2,   2])
    ...
    def initTerm(self):
        self.rot = ctrl.Consequent(np.arange(-0.5, 0.5, 0.0001), "rotation")
        ...
        self.vel = ctrl.Consequent(np.arange(-0.02, 0.22, 0.0001), "velocity")
